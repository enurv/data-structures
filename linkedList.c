...
node* sil(node* root, int x){//root degeri degisebilecegi icin yine node* gonderiyoruz.
    node* temp; //free() fonksiyonunu calistirmak icin silinen dugumun adresi gecici bir sure icin saklanmalı.
    node* iter = root;
    if(x == root -> x){ //eger aranan deger ilk root dugumundeyse
        ...
        if(iter->next == NULL){//listenin sonuna gelindiyse ve deger bulunmadiysa. (cunku iter silinecek elemanın bir onunde durmali)
